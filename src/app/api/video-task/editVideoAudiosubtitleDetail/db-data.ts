...
export const getDBJsonData = async (taskMainId: string) => {
  ...
  try {
    ...
    console.log(
        '[getDBJsonData] 步骤1 - 查询vt_task_main耗时:',
        Date.now() - step1Start, 'ms');
    if (!taskMainItem) {
    ...
    console.log(
        '[getDBJsonData] 步骤2 - 查询vt_file_original耗时:',
        Date.now() - step2Start, 'ms');
    if (!videoItem) {
    ...
    const tempFileTaskList =
        await getVtFileTaskListByTaskId(taskMainId, stepNameArr);
    console.log(
        '[getDBJsonData] 步骤3 - 查询vt_file_task耗时:',
        Date.now() - step3Start, 'ms, 数量:', tempFileTaskList.length);

    // 4、过滤步骤为"split_audio_video"且r2_key中含有video_nosound.mp4数据，当做无声视频url(私桶)
    const step4Start = Date.now();
    const noSoundVideoFile = tempFileTaskList.find(
        (file) => file.stepName === 'split_audio_video' &&
            file.r2Key.includes('video_nosound.mp4'));
    console.log(
        '[getDBJsonData] 步骤4 - 过滤无声视频耗时:', Date.now() - step4Start,
        'ms');
    ...
    //         const params = JSON.stringify({ key: noSoundVideoFile.r2Key,
    //         expiresIn: 86400 }); noSoundVideoUrl = await getJavaServer(url,
    //         params);
    //     } else {
    //         // 调用自己的接口24小时有效期
    //         noSoundVideoUrl = await
    //         generatePrivateR2SignUrl(noSoundVideoFile.r2Key, 86400);
    //     }
    // }
    // console.log('[getDBJsonData] 步骤5 - 生成无声视频URL耗时:', Date.now() -
    // step5Start, 'ms, 使用Java:', USE_JAVA_REQUEST);
    ...
    const backgroundAudioFile = tempFileTaskList.find(
        (file) => file.stepName === 'split_vocal_bkground' &&
            file.r2Key.includes('audio_bkground.wav'));
    console.log(
        '[getDBJsonData] 步骤6 - 过滤背景音频耗时:', Date.now() - step6Start,
        'ms');
    ...
    if (backgroundAudioFile) {
      if (USE_JAVA_REQUEST) {
        ...
        const url = `${JAVA_SERVER_BASE_URL}/api/r2/presigned-url`;
        const params = [
          {key: noSoundVideoFile?.r2Key, expiresIn: 86400},
          {key: backgroundAudioFile.r2Key, expiresIn: 86400}
        ];
        ...
        const resUrlArr =
            await generatePrivateR2SignUrl(r2KeyArr, 86400);  // 24小时有效期
        noSoundVideoUrl = resUrlArr[0];
        backgroundAudioUrl = resUrlArr[1];
      }
    }
    console.log(
        '[getDBJsonData] 步骤7 - 生成背景音频URL耗时:', Date.now() - step7Start,
        'ms, 使用Java:', USE_JAVA_REQUEST);
    ...
    const publicBaseUrl =
        await getSystemConfigByKey('r2.public.base_url');  // 含缓存
    console.log(
        '[getDBJsonData] 步骤10 - 查询公桶配置耗时:', Date.now() - step10Start,
        'ms');
    ...
    const allSubtitleList = await getVtTaskSubtitleListByTaskIdAndStepName(
        taskMainId, stepNameArr2);
    // const translateSrtList = await
    // getVtTaskSubtitleListByTaskIdAndStepName(taskMainId, []);
    console.log(
        '[getDBJsonData] 步骤11 - 查询字幕列表耗时:', Date.now() - step11Start,
        'ms, 原字幕和翻译长度:', allSubtitleList.length);

    // 12、过滤getSubtitleList为原字幕列表
    const step12Start = Date.now();
    // const originalSubtitleList = allSubtitleList[0]?.subtitleData || [];
    const originalSubtitleItem =
        allSubtitleList.find((item) => item.stepName === 'gen_srt');
    console.log(
        '[getDBJsonData] 步骤12 - 格式化原字幕耗时:', Date.now() - step12Start,
        'ms');

    // 13、过滤translateList为翻译字幕列表
    const step13Start = Date.now();
    const translatedSubtitleItem =
        allSubtitleList.find((item) => item.stepName === 'translate_srt');
    console.log(
        '[getDBJsonData] 步骤13 - 格式化翻译字幕耗时:',
        Date.now() - step13Start, 'ms');
 ...
 * @returns 预签名URL数组
 */
// async function generatePrivateR2SignUrl(r2KeyArr: any[] = [], expiresIn:
// number = 3600): Promise<string[]> {
//     try {
//         // 多个存储桶公用一个
//         const endpoint =
//         'https://a611f60c1436512acfe03a1efe79a50a.r2.cloudflarestorage.com';
//         const client = new S3Client({
//             region: 'auto',
//             endpoint,
//             credentials: {
//                 accessKeyId: '8c8b2be18e4795250df389d2e7cbaa2b',//
//                 每个存储桶单独id secretAccessKey:
//                 '114e0ccba3a492c6e8f76c0b057f554b8b5808d34f22bf35bfa86af0765b6d15',//
//                 每个存储桶单独secret
//             },
