...
import { updateUserWithEmail, findUserById, findUserByEmail } from '@/shared/models/user';
...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!hasUpperCase || !hasLowerCase) {
      return NextResponse.json(
        { success: false, error: 'Password must contain both uppercase and lowercase letters' },
        { status: 400 }
      );
    }
    ...
    if (!currentEmail.startsWith('guest_') || !currentEmail.endsWith('@temp.local')) {
      return NextResponse.json(
        { success: false, error: 'Not a guest account' },
        { status: 400 }
      );
    }
