...
export async function findVtFileOriginalById(id: string) {
  const [result] =
      await db()
          ...
          .from(vtFileOriginal)
          .where(
              and(eq(vtFileOriginal.id, id), eq(vtFileOriginal.delStatus, 0)))
          .limit(1);
  return result;
...
export async function updateVtFileOriginal(
    id: string, data: Partial<NewVtFileOriginal>) {
 ...
 * @returns
 */
export async function updateVtFileOriginalCoverTitle(
    id: string, title: string, coverKey: string, coverSize: number) {
  const [result] = await db()
                       .update(vtFileOriginal)
                       .set({
                         fileName: title,
...
export async function getVtFileOriginalList(
  ...
  return await db()
      ...
      .from(vtFileOriginal)
      .where(and(
          eq(vtFileOriginal.userId, userId), eq(vtFileOriginal.delStatus, 0)))
...
export async function getVtFileOriginalTotal(userId: string) {
  const [result] = await db()
                       ...
                       .from(vtFileOriginal)
                       .where(
                           and(eq(vtFileOriginal.userId, userId),
                               eq(vtFileOriginal.delStatus, 0)));
