...
为保持视频编辑器整体一致性，这组状态 UI 在实现时需要额外遵守：

- 继续沿用当前工作台的工具型视觉语言，不引入新的卡片式子面板
- 状态表达压缩为：`小状态标签 + 一行辅助说明 + 轻量按钮`
- 动作按钮优先复用现有 `Button` 体系的 `outline / ghost / default` 风格
- 不新增厚重阴影、强渐变、大面积彩色底块
- 播放按钮、时间 pill、选中态仍以现有行样式为主，只在必要处做轻微状态差异

这意味着“状态要清楚”，但“视觉存在感要低于编辑器本身的主结构”。
