...
- API 入口：`src/app/api/video-task/generate-subtitle-voice/route.ts`（`type=gen_srt`）
- Java 调用封装：`src/shared/services/javaService.ts`（`javaSubtitleSingleTranslate`）
...
- **Method**：`POST`
- **Path**：`/api/nextjs/subtitle/single/translate`
  - 注意：本仓库代码里会请求：`${JAVA_SERVER_BASE_URL}/api/nextjs/subtitle/single/translate`
  - 如果 Java 部署使用了 `context-path=/video`，请把 `JAVA_SERVER_BASE_URL` 配成包含 `/video` 的 base（例如 `http://localhost:18412/video`）
- **通信安全**：
...
```ts
const url = `${JAVA_BASE_URL}/api/nextjs/subtitle/single/translate`;
...
- `body` 为密文字符串：`EncryptionUtil.encryptRequest(data)` 的输出
...
加密协议与实现参考：
- Next.js：`src/shared/lib/EncryptionUtil.ts`（使用 `ENCRYPTION_SECRET`）
- Java：video-tools 内部的解密 filter（需与 `ENCRYPTION_SECRET` 保持一致）
